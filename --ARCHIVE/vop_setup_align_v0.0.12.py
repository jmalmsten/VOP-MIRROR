"""
VOP Module:     vop_setup_align_v0.0.12.py
Version:        v0.0.12
Description:    Alignment Utility using Port 5555 to avoid Codium conflicts.
"""
...
VOP_PORT = "5555" # Changed to avoid desktop conflicts

def prepare_system():
    print("1. [CLEANUP] Killing ghost camera processes...")
...
def start_stream():
    print(f"3. [STREAM] Pushing UDP to {DESKTOP_IP}:{VOP_PORT}...")
    # Switched back to UDP push for simpler logic now that port is clear
    cmd = [
        "sudo", "rpicam-vid", "-t", "0", "--inline", 
        "--width", "1280", "--height", "720",
        "--framerate", "30", "--codec", "h264", 
        "--profile", "baseline", "--flush", "--tune", "zerolatency",
        "-o", f"udp://{DESKTOP_IP}:{VOP_PORT}?pkt_size=1316"
    ]
...
def run():
    ...
    try:
        ...
        print(f"FATAL: {e}"); stream_proc.terminate(); pygame.quit(); sys.exit(1)
    ...
    print(f"\n✅ LIVE. Sending to Desktop Port {VOP_PORT}")
    running = True
    while running:
