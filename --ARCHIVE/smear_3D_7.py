"""
VOP Module:     smear_3D_7.py
Version:        v0.0.7
Description:    3D Smear Engine optimized for Pi 5 GLES 3.1.
                - Robust subprocess handling for rpicam-still.
                - Full non-truncated capture loop.
                - Clinical feedback for camera trigger states.
"""
...
def run_vop_3d(args):
    ...
    CAM_MAG_DIR = "/home/admininja/vop/CamMag" # Ensure results go here
    VERSION = "v0.0.7"
    ...
    os.environ["SDL_DRM_DEVICE"] = "/dev/dri/card0" if os.path.exists("/dev/dri/card0") else "/dev/dri/card1"
    ...
    try:
        ...
        print("\r\n--- 3D VOP v0.0.7 ---")
        print("Arrows: S/M/E | Enter: Run | Ctrl+C: Exit\r\n")
    ...
    while True:
        ...
        # Wait until the smear is done + small buffer for camera to save
        if elapsed > (TOTAL_SMEAR + 2000): 
            break
        ...
        if not captured and elapsed >= (1000.0 - args.offset):
            print(f"TRIGGERING CAMERA: {SHUTTER_US}us shutter...")
            try:
                subprocess.Popen([
    ...
    print(f"CAPTURE SEQUENCE FINISHED. File: {output_file}")
    pygame.quit()
