"""
VOP Module:     vop_setup_align_v0.0.2.py
Version:        v0.0.2
Description:    All-in-one Setup & Alignment Utility.
                Surgical process cleanup and low-latency focus feed.
"""
...
def prepare_system():
    """Cleans background tasks and grabs HDMI plane."""
    print("🧹 Cleaning up background processes...")
    my_pid = os.getpid()
    
    # Kill rpicam instances
    subprocess.run("sudo killall -9 rpicam-vid rpicam-still 2>/dev/null", shell=True)
    
    # Kill other python processes, but NOT this one (Surgical Kill)
...
def start_stream():
    ...
    # Using baseline profile and frequent keyframes to prevent lag buildup
    cmd = [
