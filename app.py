...
VOP Module:     app.py
Version:        v0.7.1
Description:    Phase V Orchestrator. Final UI layout flow.
"""
...
DEFAULT_FILE = os.path.join(BASE_DIR, "default_job.json")
...
progress_state = {"current": 0, "total": 0, "msg": "Idle", "status": "idle", "latest_wp": ""}

def init_state():
    for d in [CAM_MAG_DIR, WORKPRINT_DIR]: os.makedirs(d, exist_ok=True)
    if not os.path.exists(DEFAULT_FILE):
        baseline = {
            "v": 5, "last_sync": 0, "f1": "1", "f2": "24", "f3": "48", 
            "image": "vop_logo_mockup_400px.png", "c1_hex": "#ffffff", 
            "fov": "45", "gain": "12", "awb_r": "3.18", "awb_b": "1.45", 
            "fps": "24", "tiff_compression": "zip", "probe_frame": "1", "probe_sub": "0.5"
        }
        with open(DEFAULT_FILE, 'w') as f: json.dump(baseline, f, indent=4)
    if not os.path.exists(CURRENT_FILE): shutil.copy(DEFAULT_FILE, CURRENT_FILE)
...
def preview():
    ...
    job = {**data, "p_start": ",".join(map(str, ps['p'])), "p_end": ",".join(map(str, ps['p'])),
...
def execute():
    ...
    f1, f3 = int(data['f1']), int(data['f3'])
    total = (f3 - f1) + 1
    progress_state.update({"current": 0, "total": total, "status": "running", "msg": "Exposing...", "latest_wp": ""})
    ...
    progress_state.update({"status": "idle", "msg": "COMPLETE", "latest_wp": wp_name})
    return jsonify({"status": "SUCCESS"})
