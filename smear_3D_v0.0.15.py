"""
VOP Module:     smear_3D_v0.0.15.py
Version:        v0.0.15
Description:    Phase II/III - 16-bit Linear Restoration.
                - Implements 0.5s Black / X s Smear / 0.5s Black logic.
                - Forces 16-bit uncompressed TIFF output (~74MB).
                - Supports UI-driven Gain and AWB.
"""
...
# Hard-coded latency offset from previous calibration days
...
def run_vop_engine(job_path):
    ...
    if is_preview:
        ...
        # Open Shutter (using UI Gain and AWB)
        ...
        while True:
            ...
            # Animation Logic (Centered in the 1s total black buffer)
        ...
        # --- THE 74MB FIX (FORCE 16-BIT) ---
        if os.path.exists(buffer_file):
            new_img = cv2.imread(buffer_file, cv2.IMREAD_UNCHANGED)
            # Fail-safe: bit-shift 8-bit to 16-bit to preserve math headroom
            if new_img.dtype == np.uint8:
                new_img = (new_img.astype(np.uint16) * 256)

            if os.path.exists(output_file):
                latent = cv2.imread(output_file, cv2.IMREAD_UNCHANGED)
                if latent.dtype == np.uint8: latent = (latent.astype(np.uint16) * 256)
