...
def run_vop_engine(job_path):
    ...
    pygame.init(); pygame.mouse.set_visible(False)
    ...
    tex = ctx.texture(pygame.image.load(os.path.join(proj_mag_dir, job['image'])).get_size(), 4, 
                      pygame.image.tostring(pygame.image.load(os.path.join(proj_mag_dir, job['image'])).convert_alpha(), "RGBA", True))
