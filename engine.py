...
Version:        v0.0.60-stable
Description:    Fixed video init race condition. Full smear previews.
"""
...
# Force Pi to use the correct display driver
...
def save_frame_async(buffer_file, output_file, tiff_flag, cam_gel_rgb, frame_num):
    try:
        ...
        # Touch heartbeat file to signal frame completion to the app.py estimator
...
def run_vop_engine(job_path):
    ...
    # Hard-init Pygame before any logic
    pygame.init()
    # Retry loop for display init to prevent "video system not initialized"
    screen = None
    for _ in range(5):
        try:
    ...
    if not screen:
        print("CRITICAL: Could not initialize video system.")
        sys.exit(1)
    ...
    prog['mono_mode'].value = (job.get('mono_mode') == 'on')
    res_str = job.get('cam_res', '2028x1520')

    def render_smear(frame_data, buffer_file, output_file, is_preview=False):
        p1, p2 = [np.array([float(x) for x in frame_data[k].split(',')]) for k in ['p_start', 'p_end']]
        r1, r2 = [np.array([float(x) for x in frame_data[k].split(',')]) for k in ['r_start', 'r_end']]
        c1, c2 = np.array(frame_data['c_start']), np.array(frame_data['c_end'])
        cg_start, cg_end = np.array(frame_data['cg_start']), np.array(frame_data['cg_end'])
        
        x_ms = float(frame_data['smear']) * 1000.0
        total_ms = x_ms + 1000.0
        
        cam_proc = hw.trigger_capture(buffer_file, total_ms, job['gain'], job['awb_r'], job['awb_b'], res_str)
        ...
        if is_preview:
            ...
            avg_cg = (cg_start + cg_end) / 2.0
            img = (img.astype(np.float32) * [avg_cg[2], avg_cg[1], avg_cg[0]]).clip(0, 255).astype(np.uint8)
            # LIME check for preview
            if os.path.exists(output_file):
                disk = cv2.imread(output_file)
                if disk is not None: img = cv2.add(img, cv2.resize(disk, (img.shape[1], img.shape[0])))
            cv2.imwrite(os.path.join(static_dir, "probe_live.jpg"), img)
            if os.path.exists(buffer_file): os.remove(buffer_file)
        else:
            avg_cg = (cg_start + cg_end) / 2.0
            save_frame_async(buffer_file, output_file, 8 if job.get('tiff_compression') == 'zip' else 1, avg_cg, frame_data.get('frame', 0))

    # EXECUTION MODES
    if job.get('type') == 'preview':
        # Point-in-time projector probe (No Camera)
        ctx.clear(0,0,0); ctx.viewport = (0, 0, WIDTH, HEIGHT)
        p, r, c = np.array([float(x) for x in job['p_start'].split(',')]), np.array([float(x) for x in job['r_start'].split(',')]), np.array(job['c_start'])
        proj = Matrix44.perspective_projection(float(job['fov']), WIDTH/HEIGHT, 0.1, 1000.0)
    ...
    elif job.get('type') == 'cam_preview':
        # Full Hardware Smear Preview
        target_f = os.path.join(cam_mag_dir, f"latent_{str(job.get('probe_frame','0001')).zfill(4)}.tif")
        render_smear(job, "/tmp/vop_prev_buf.dng", target_f, is_preview=True)

    else:
        # Sequence Loop
        frames = job.get('sequence', [])
        if os.path.exists("/tmp/vop_heartbeat"): os.remove("/tmp/vop_heartbeat")
        for f_job in frames:
            out_f = os.path.join(cam_mag_dir, f"latent_{str(f_job['frame']).zfill(4)}.tif")
            render_smear(f_job, f"/tmp/vop_buf_{f_job['frame']}.dng", out_f)
